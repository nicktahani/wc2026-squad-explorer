...
export default function GroupStage({ groups, matches, teamsByName }) {
  ...
  for (const match of matches.filter(match => match.stage === 'group')) {
    if (!matchesByGroup.has(match.group)) matchesByGroup.set(match.group, [])
    matchesByGroup.get(match.group).push(match)
  }

  return (
    <section className="group-stage-section" aria-labelledby="group-stage-title">
      <header className="group-stage-section__header">
        <h2 id="group-stage-title" className="group-stage-section__title">Group Stage</h2>
      </header>
